        ...
        while (true) {
            ...
            if (in.equalsIgnoreCase("1")) {
                ...
                if (!jinguBuffStack.isEmpty()) {
                    ...
                    jinguBuffStack.pop();
                    System.out.println(" ");
                    System.out.println("Jingu Mastery buff! +80% bonus damage.");
                    if (jinguBuffStack.isEmpty()) {
                        System.out.println("Jingu Mastery buff has worn off!");
                        jinguMastery = 4; // Reset stacks after buff ends
                    }
                }

                // If buff is not active, decrement Jingu stacks on each attack
                if (jinguMastery > 0 && jinguBuffStack.isEmpty()) {
                    jinguMastery--;
                    // When stacks reach 0, activate buff for 4 turns using stack
                    if (jinguMastery == 0) {
                        for (int i = 0; i < 4; i++) {
                            jinguBuffStack.push(1); // Push a dummy value for each turn of buff
                        }
                        System.out.println(" ");
                        System.out.println("Jingu Mastery buff activated! +80% bonus damage for the next 4 turns!");
                    }
                }
                System.out.println(" ");
                ...
                System.out.println("Absolute Defense Activated");
                absoluteDef = true;
                ...
                if (absoluteDef) {
                    double reduction = dmgReduction[1]; // 0.8 → 80% reduction, take only 20%
                    int reducedDmg = (int) Math.round(botDmg * (1 - reduction));
                    playerHP -= reducedDmg;
                    absoluteDef = false;
                    System.out.println("Absolute Defense! Damage reduced by 80%: You took " + reducedDmg + " damage.");
                } else {
                    playerHP -= botDmg;
